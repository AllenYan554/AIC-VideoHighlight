"""subject localization Full Dev inference: resumable per-video RT-DETR localization over Dev166."""
...
from aic_video_highlight.localization import (
    DEFAULT_MODEL_ID,
    RTDetrLocalizer,
    SubjectCandidate,
    SubjectPolicyConfig,
...
def validate_prediction_manifest(
    ...
    *,
    ...
    """Validate the frame projection prediction manifest before inference.
...
def run(args: argparse.Namespace) -> int:
    ...
    for video_id in sorted(predictions):
        ...
        if shard_is_complete(output_dir, video_id, frame_ids):
            ...
            print(
                f"[localization] video {finished}/{total_videos} {video_id} resumed=yes "
        ...
        if not frame_ids:
            ...
            eta = elapsed * (total_videos - finished) / finished if finished else 0.0
            print(
                f"[localization] video {finished}/{total_videos} {video_id} frames=0 empty=yes "
                f"completed={processed} skipped={skipped} failed={model_error_frames} "
                f"elapsed={elapsed:.0f}s eta={eta:.0f}s",
                flush=True,
            )
        ...
        eta = elapsed * (total_videos - finished) / finished if finished else 0.0
        print(
            f"[localization] video {finished}/{total_videos} {video_id} "
