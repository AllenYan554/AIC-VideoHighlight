"""spatial composition CMP-1 geometry: primary-bbox sanitization and subject-shifted crop.

CMP-1 keeps the frame projection frozen maximal legal target-ratio crop SIZE and only
changes the crop POSITION (single-variable control vs CMP-0 Center Crop).
The frame projection ``compute_center_crop`` function is reused verbatim for the crop
...
def sanitize_primary_bbox(
    ...
    ``raw_bbox`` is the frozen subject localization primary xyxy box. If the clamped box
...
def projection_crop_height(width: int, height: int, target_w: int | float, target_h: int | float) -> int:
    """frame projection internal crop height implied by the frozen maximal legal crop.
...
def compute_subject_shifted_crop(
    ...
    The crop size (crop_w, official derived height) is exactly the frame projection
    frozen maximal legal crop from ``compute_center_crop``; only x/y placement
    changes. Ideal placement uses the frame projection internal centering convention
