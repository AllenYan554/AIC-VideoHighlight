"""temporal stabilization temporal stability evaluation over frozen TS-0 (CMP-1) vs TS-1 records.

Pure evaluation of merged per-frame records produced by the temporal stabilization runner:
...
def observations_from_records(records: Sequence[Mapping[str, Any]]) -> list[TemporalObservation]:
    ...
    the frozen spatial composition placement consumed). Fallback frames carry the frozen
...
def temporal_stability_metrics(records: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "metric_role": "DESCRIPTIVE_ONLY / no promotion gate in temporal stabilization v1",
    }
...
def spatial_guardrail_metrics(records: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    """spatial composition spatial guardrails recomputed for TS-0 vs TS-1."""
...
def temporal_multi_subject_diagnostic(
    ...
    Mirrors the frozen spatial composition diagnostic semantics: ambiguous frames only,
...
def summarize_multi_subject_rows(
    ...
    if candidate_side not in {"ts1", "ts2", "ts3", "ts4", "ts5"}:
