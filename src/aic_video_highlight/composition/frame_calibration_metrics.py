"""prediction selection weak-frame temporal proxy metrics, guardrails and decisions.

The official competition metric couples frame selection with same-frame spatial
IoU.  prediction selection freezes the spatial side and studies only frame cardinality, so
...
from aic_video_highlight.composition.frame_selection import FS0, FS1, FS2
...
class FrameCalibrationError(ValueError):
    """Raised when a prediction selection metric or decision input is malformed."""
...
def adjudicate_prediction_selection(
    ...
    if dev_winner is None:
        return {
            ...
            "frozen_policy": FS0,
            "prediction_selection_status": "PREDICTION_SELECTION_CLOSED",
    ...
    if hard_result is None:
        return {
            ...
            "prediction_selection_status": None,
