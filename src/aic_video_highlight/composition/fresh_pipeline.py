"""VHiCraft-v1 true-fresh deterministic core (inference corrective integration).
...
The frozen temporal refinement candidate cache (``4b515a6d...``) is deliberately NOT
...
TS0_METHOD = "composition_final_frozen_cmp1"
FRESH_SHARD_SCHEMA_VERSION = "aic.vhicraft.stabilization-shard/v1"
FRESH_CACHE_BINDING_SCHEMA_VERSION = "aic.vhicraft.cache-binding/v1"
...
def validate_fresh_candidate_cache_binding(
    ...
    *,
    retrieval_predictions_path: Path,
    expected_video_ids: Sequence[str],
    expected_split: str,
    forbidden_global_sha256: str,
) -> dict[str, Any]:
    """Prove that a candidate cache was built from this run's retrieval output.
    ...
    if not isinstance(source_sets, list) or len(source_sets) != 1:
        raise FreshPipelineError("fresh cache must have exactly one source set")
    source = source_sets[0]
    if source.get("split") != expected_split:
        raise FreshPipelineError(
            f"fresh cache source split mismatch: expected {expected_split}"
        )
    hashes = source.get("source_artifact_hashes", {})
    predictions_sha = file_sha256(retrieval_predictions_path)
    if hashes.get("predictions_jsonl_sha256") != predictions_sha:
        raise FreshPipelineError("fresh cache does not consume this run's retrieval predictions")
...
def build_fresh_index(
    ...
    *,
    ...
    """Project frozen Dev membership onto a fresh release index.
...
def build_shard_records(
    ...
    """Serialize one video's fresh temporal stabilization shard (``ts0`` = CMP-1, ``ts5`` = TS-5 Revised).

    Schema-compatible with ``vhicraft_pipeline.load_stabilization_shard`` so inference
