"""temporal stabilization TS-1 fixed, TS-2 adaptive, TS-3 center-constrained, and
TS-4 bbox-aware and TS-5 projected-state constrained EMA methods.

Consumes the spatial composition FINAL FROZEN CMP-1 geometry as TS-0 (temporal control
...
fallback semantics are all inherited unchanged from the frozen spatial composition contract.
...
center. All are placed with the exact spatial composition frozen floor/clamp convention.
...
def motion_adaptive_alpha(
    ...
    *,
    ...
    ``motion_norm`` is the frozen temporal stabilization horizontal motion signal
...
def place_crop_from_center(
    ...
    """Frozen spatial composition placement of the maximal legal crop at an arbitrary center.

    Identical floor/clamp convention as ``compute_subject_shifted_crop``: the crop
    size is the frame projection maximal legal target-ratio crop, the ideal placement uses
    ...
    crop_w = center_box.w
    crop_h = projection_crop_height(width, height, target_w, target_h)
...
def large_jump_ratios(
    ...
    temporal stabilization v1 protocol they must not be turned into a promotion gate.
    """
