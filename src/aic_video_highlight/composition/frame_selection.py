"""prediction selection frame-level prediction calibration (emit/drop selection).

This module changes ONLY which candidate frames that already own a frozen
temporal stabilization TS-5 Revised bbox are emitted.  It never changes frame identity,
...
All temporal-segment -> frame projections reuse the frozen frame projection canonical
mapping (:func:`aic_video_highlight.composition.frame_projection`
...
class FrameSelectionError(ValueError):
    """Raised when prediction selection temporal-evidence inputs are malformed."""
...
@dataclass(frozen=True, slots=True)
class FinalSegment:
    """One frozen temporal refinement final segment (identity plus frozen time span)."""
...
def compute_support_by_frame(
    ...
    """Compute exact ``E_t`` / ``S_t`` for each candidate frame via frame projection."""
...
def select_segment_frames(
    ...
    if policy not in POLICIES:
        raise FrameSelectionError(f"unknown prediction selection policy: {policy}")
