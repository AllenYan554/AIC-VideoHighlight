"""VHiCraft-v1 pipeline helpers: inference validation and final freeze.
...
it integrates the already FINAL_FROZEN retrieval..5.5 modules, assembles the
official-format predictions, and provides the preregistered comparison and
invariant checks.

Inference variants (internal validation):

* ``stabilized`` -- full pipeline with TS-5 Revised temporal stabilization.
* ``unstabilized_control`` -- identical upstream with TS-0 (CMP-1) instead of
  TS-5 Revised; the only difference is temporal stabilization.
"""
...
from aic_video_highlight.composition.submission import build_submission_record
...
UNSTABILIZED_CONTROL = "unstabilized_control"
VARIANTS = (STABILIZED, UNSTABILIZED_CONTROL)
# temporal stabilization output geometry key consumed by each variant.
ARM_STABILIZATION_KEY = {STABILIZED: "ts5", UNSTABILIZED_CONTROL: "ts0"}
...
class VHiCraftPipelineError(ValueError):
    """Raised when an inference identity or reproduction invariant is broken."""
...
def load_stabilization_shard(shard_path: Path) -> dict[int, dict[str, FrameCrop]]:
    """Read one temporal stabilization per-video shard into ``frame -> {ts0/ts5: crop}``."""
...
def fs0_identity_holds(before_frames: Iterable[int], after_frames: Iterable[int]) -> bool:
    """prediction selection FS-0 must be a pure identity: identical frame sets and order."""
...
def ablation_invariants(
    ...
    if set(ts0_crops) != set(ts5_crops):
        raise VHiCraftPipelineError("temporal stabilization ablation changed the frame set")
    width_changes = 0
    ...
    if width_changes:
        raise VHiCraftPipelineError(
            f"temporal stabilization ablation changed crop width for {width_changes} frames"
        )
    moved = sum(
        1
        for frame in ts0_crops
...
DETERMINISM_POLICY = {
    ...
    "B_deterministic_cpu_transforms": {
...
def build_final_pipeline_manifest(
    *,
    ...
    return {
        "schema_version": "aic.vhicraft.pipeline-manifest/v1",
