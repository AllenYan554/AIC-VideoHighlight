"""Spatial composition pipeline: frozen-input loading, manifest, CMP-0/CMP-1 composition.

CMP-0 = frame projection Frozen Center Crop (control, taken verbatim from the frozen
predictions artifact). CMP-1 = Primary-Subject Shifted Max-Ratio Crop: the frame
projection maximal legal crop size with position shifted toward the sanitized
primary subject center. No detector inference, no frame reselection, no subject
research.
"""
...
@dataclass(frozen=True, slots=True)
class FrozenInputs:
    frame_projection_predictions: dict[str, dict[str, Any]]
...
def load_policy_shard_dir(path: Path) -> list[dict[str, Any]]:
    """Load frozen subject localization policy decisions from the canonical per-video shard layout."""
    if not path.is_dir():
...
def policy_shard_semantic_sha(records: Sequence[Mapping[str, Any]]) -> str:
    ...
    from aic_video_highlight.localization.full_dev import canonical_sha

    return canonical_sha(records)


def load_raw_shard_dir(path: Path) -> dict[tuple[str, int], dict[str, Any]]:
    """Load frozen subject localization raw detector records from the canonical shard layout."""
    if not path.is_dir():
...
def verify_shard_dir_integrity(
    ...
    """Recheck every frozen subject localization shard status (frame count + byte hashes)."""
    from aic_video_highlight.localization.full_dev import shard_is_complete
...
def load_frozen_inputs(
    ...
    *,
    ...
    """Load and hash-verify frozen frame projection/5.2 inputs (read-only)."""
    ...
    if include_raw:
        ...
        if raw_binding.format == "raw_shard_dir":
            ...
            raw_frames = {
                ...
                for frame in payloads["localization_raw_detector"]["frames"]
            }
...
def _cross_check_inputs(
    ...
    for video_id, record in index.items():
        if video_id not in predictions:
            raise FrozenInputError(f"index video missing from frame projection predictions: {video_id}")
...
def build_manifest(
    ...
    *,
    ...
    """Deterministic, model-blind manifest from frozen frame projection/5.2 artifacts.

    Frame set = exact frames of the frozen subject localization formal policy artifact
    ...
    for video_id in sorted(policy_by_video):
        ...
        predicted = {int(pred["frame"]) for pred in inputs.frame_projection_predictions[video_id]["predictions"]}
        entries = []
        for record in sorted(policy_by_video[video_id], key=lambda item: int(item["frame"])):
            frame = int(record["frame"])
            if frame not in predicted:
                raise FrozenInputError(f"subject localization frame outside the frame projection frozen set: {(video_id, frame)}")
            ...
            entries.append(
                {
                    "frame": frame,
                    "stratum": stratum,
                    "localization_status": record["status"],
    ...
    manifest: dict[str, Any] = {
        ...
        "model_blind": True,
        "selection_rule": (
            "frames = exact frame set of the frozen subject localization formal policy artifact "
            "(sorted video_id, ascending frames); strata = preregistered horizontal "
            "center-offset thresholds on the sanitized subject localization primary bbox; "
...
def compose_frame(
    ...
    if frozen_boxes is None:
        frozen_boxes = {
            ...
            for pred in inputs.frame_projection_predictions[video_id]["predictions"]
        }
    ...
    if is_fallback:
        cmp1 = {
            ...
            "crop_w": frozen_w,
            "crop_h": projection_crop_height(width, height, tw, th),
    ...
    return {
        ...
        "image_width": width,
        "image_height": height,
        "localization_status": policy["status"],
...
def compose_all_frames(
    ...
    for video in manifest["videos"]:
        frozen_boxes = {
            int(pred["frame"]): list(pred["bboxes"])
            for pred in inputs.frame_projection_predictions[video["video_id"]]["predictions"]
        }
...
def aggregate_metrics(
    ...
    return {
        ...
        "fallback": {
            ...
            "localization_fallback_reason_counts": dict(sorted(fallback_reasons.items())),
        },
        "selected_primary_overflow_audit": {
            "scope": "frozen subject localization formal policy artifact frames (selected primary bboxes only)",
...
def mirror_reliable_candidate(
    ...
    Returns the candidate the frozen subject localization policy would rank first among
    ...
    frozen subject localization decisions are never modified.
    """
    pool = [
...
def box_too_large_diagnostic(
    ...
    return {
        "tag": "FULL_FRAME_SUBJECT_DIAGNOSTIC / observation only / subject localization decisions NOT modified",
...
def fallback_reason_diagnostic(
    ...
    """Per-reason observation of frozen subject localization fallbacks: count/ratio/class/area.
    ...
    return {
        "tag": "FALLBACK_REASON_DIAGNOSTIC / observation only / frozen subject localization semantics untouched",
        "total_frames": total,
