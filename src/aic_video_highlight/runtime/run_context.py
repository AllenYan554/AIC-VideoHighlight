...
from .paths import RunPaths, validate_run_id
...
def _package_versions() -> dict[str, str | None]:
    ...
    for distribution in ("vhicraft", "PyYAML", "openai"):
        try:
...
class RunContext:
    ...
    def start(self, *, resume: bool = False) -> dict[str, Any]:
        ...
        if self.manifest_path.exists():
            ...
            manifest = {
                "schema_version": "aic.vhicraft.run-manifest/v1",
                **identity,
                "started_at": utc_now(),
                "finished_at": None,
    ...
    def set_status(self, status: str, **details: Any) -> None:
        ...
        payload = {"schema_version": "aic.vhicraft.run-status/v1", "status": status, "updated_at": utc_now(), **details}
