...
def chunk_heartbeat_line(
    video_id: str, chunk_index: int, chunk_total: int, phase: str
) -> str:
    """Return one watchdog-visible, numerics-neutral chunk boundary line."""
    if phase not in {"begin", "end"}:
        raise ValueError("heartbeat phase must be begin or end")
    if chunk_total <= 0 or not 0 <= chunk_index < chunk_total:
        raise ValueError("invalid heartbeat chunk index/total")
    return (
        f"[vhicraft] heartbeat video_id={video_id} "
        f"chunk={chunk_index + 1}/{chunk_total} phase={phase}"
    )
...
def parse_saved_raw_outputs(
    ...
    *,
    ...
    """Re-run parser and merge components from persisted responses without model calls."""
...
class HighlightRetrievalPipeline:
    ...
    def _analyze_chunk(
        ...
        *,
        ...
        heartbeat_video_id: str | None = None,
        chunk_total: int = 1,
        ...
        if heartbeat_sink is not None:
            heartbeat_sink(
                chunk_heartbeat_line(
                    heartbeat_video_id or media.stem,
                    chunk.index,
                    chunk_total,
                    "begin",
                )
        ...
        *,
        ...
        if len(chunks) == 1:
            segments, raw_record, parse_time_sec = self._analyze_chunk(
                ...
                heartbeat_video_id=heartbeat_video_id or meta.video_id,
