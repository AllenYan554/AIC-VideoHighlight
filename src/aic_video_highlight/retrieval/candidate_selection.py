"""Deterministic candidate selection filtering over the frozen candidate cache.
...
ROLE_MANIFEST_SCHEMA_VERSION = "aic.vhicraft.dev-role-manifest/v1"
ROLE_MANIFEST_SUMMARY_SCHEMA_VERSION = "aic.vhicraft.dev-role-manifest-summary/v1"
...
class CandidateSelectionError(ValueError):
    """Raised when a candidate selection isolation or selection invariant is broken."""
...
def partition_cache_records(
    ...
    for item in audit_membership:
        ...
        # outside the candidate selection data-isolation contract.
        video_id = item.get("video_id")
        split = item.get("split")
...
def create_role_manifest(
    *,
    ...
    if role not in EXPECTED_ROLE_COUNTS:
        raise CandidateSelectionError("unknown candidate selection role")
    identities = [
    ...
    *,
    ...
    evaluation: dict[str, Any] = {
        ...
        "metric_implementation": (
            "aic_video_highlight.retrieval.baseline_experiment."
...
ROLE_SUMMARY_FILENAME = "dev_role_manifest_summary.json"
PROTOCOL_SCHEMA_VERSION = "aic.vhicraft.candidate-selection-protocol/v1"
...
def load_experiment_protocol(path: Path) -> dict[str, Any]:
    ...
    if protocol.get("protocol_schema_version") != PROTOCOL_SCHEMA_VERSION:
        raise CandidateSelectionError("unsupported candidate selection protocol schema")
...
def _load_cache_payloads(
    ...
    if summary["global_semantic_sha256"] != EXPECTED_CACHE_GLOBAL_HASH:
        raise CandidateSelectionError("Frozen Cache global hash is not the candidate cache frozen hash")
