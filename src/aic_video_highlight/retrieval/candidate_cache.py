...
This module consumes persisted retrieval model responses.  It never reads video
...
OFFICIAL_TEST_DATASET_NAME = "aic_official_test"
OFFICIAL_TEST_DATASET_VERSION = "v1"
FROZEN_MODEL_NAME = "Qwen/Qwen3.5-4B"
FROZEN_PROMPT_VERSION = "high_recall_retrieval_v0"
ALLOWED_SPLITS = ("dev", "hard", "test")
_SPLIT_ORDER = {split: index for index, split in enumerate(ALLOWED_SPLITS)}


def _dataset_version_for_split(split: str) -> str:
    if split == "test":
        return OFFICIAL_TEST_DATASET_VERSION
    if split in ("dev", "hard"):
        return FROZEN_DATASET_VERSION
    raise CandidateCacheError(f"unknown candidate-cache split: {split}")


class CandidateCacheError(ValueError):
    """Raised when frozen inputs or a cache violate the candidate cache contract."""
...
def _validate_frozen_run_config(run_config: Mapping[str, Any], split: str) -> None:
    expected = {
        "split": split,
        "dataset_version": _dataset_version_for_split(split),
        "model_name": FROZEN_MODEL_NAME,
        "prompt_version": FROZEN_PROMPT_VERSION,
    }
    if split == "test":
        expected["dataset_name"] = OFFICIAL_TEST_DATASET_NAME
...
def build_video_cache_record(
    *,
    ...
    if split not in ALLOWED_SPLITS:
        raise CandidateCacheError("candidate cache accepts only dev, hard, and test")
...
def export_candidate_cache(
    ...
    *,
    ...
    """Export an immutable cache from explicit Dev/Hard/Test experiment roots."""
    if not sources:
        raise CandidateCacheError("at least one dev/hard/test source is required")
    ...
    for split in sorted(sources, key=_SPLIT_ORDER.__getitem__):
        ...
        _validate_frozen_run_config(run_config, split)
        dataset_versions.add(str(run_config["dataset_version"]))
    ...
    if len(dataset_versions) != 1:
        raise CandidateCacheError("candidate cache cannot mix dataset versions")
...
def _validate_record(record: dict[str, Any]) -> None:
    ...
    expected_dataset_version = _dataset_version_for_split(str(record.get("split")))
    if provenance.get("dataset_version") != expected_dataset_version:
        raise CandidateCacheError(f"wrong dataset version for {video_id}")
...
def validate_cache(cache_dir: Path) -> dict[str, Any]:
    ...
    if manifest.get("cache_schema_version") != CACHE_SCHEMA_VERSION:
        raise CandidateCacheError("unsupported cache schema version")
    if manifest.get("dataset_version") not in {
        FROZEN_DATASET_VERSION,
        OFFICIAL_TEST_DATASET_VERSION,
    }:
        raise CandidateCacheError("cache manifest has an unsupported dataset version")
    ...
    for entry in records:
        ...
        if record["source_provenance"]["dataset_version"] != manifest["dataset_version"]:
            raise CandidateCacheError(f"manifest/record dataset version mismatch for {video_id}")
    ...
    for source_set in source_sets:
        ...
        source_provenance = source_set.get("source_provenance")
        if (
            not isinstance(source_provenance, dict)
            or source_provenance.get("dataset_version") != manifest["dataset_version"]
        ):
            raise CandidateCacheError("source set dataset version is inconsistent")
...
def compare_identity(
    ...
    *,
    ...
    if set(sources) - set(ALLOWED_SPLITS):
        raise CandidateCacheError("identity comparison accepts only dev, hard, and test sources")
    manifest = _read_json_object(root / "cache_manifest.json")
