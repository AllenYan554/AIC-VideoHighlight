...
from aic_video_highlight.retrieval.baseline_experiment import (
    RawOutputJournal,
    atomic_write_json,
...
from aic_video_highlight.retrieval.temporal_metrics import (
    ZERO_DURATION_EPSILON_SEC,
)
from aic_video_highlight.retrieval.video_chunker import build_chunks
from aic_video_highlight.retrieval.video_clip import extract_video_clip
...
def evaluate_reference_policy(
    ...
    *,
    evaluator: Callable[..., dict[str, Any]] = evaluate_weak_references,
) -> dict[str, Any]:
    """Evaluate development references, but never inspect hidden Official-Test GT."""
    if sample.get("split") == "test":
        return {"status": "UNAVAILABLE", "reason": "HIDDEN_GT", "metrics": {}}
    return {
        "status": "AVAILABLE",
        "reason": "WEAK_REFERENCE_ONLY",
        "metrics": evaluator(predictions, sample["weak_reference_segments"]),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--local-quantization", choices=["bnb-nf4"], default="bnb-nf4")
    parser.add_argument("--local-compute-dtype", choices=["float16", "bfloat16"], default="float16")
    ...
    parser.add_argument(
        "--runtime-profile",
        type=Path,
        default=Path(__file__).resolve().parents[3] / "configs" / "runtime" / "default.json",
    )
...
def _build_result(
    ...
    *,
    ...
    metrics = dict(reference_policy["metrics"])
    warnings: list[str] = []
    if metrics.get("zero_duration_reference_count"):
    ...
    result = {
        ...
        "reference_evaluation": dict(reference_policy),
    ...
    if reference_policy["status"] == "AVAILABLE":
        result["weak_reference_segments"] = sample["weak_reference_segments"]
    return result
...
def _recover_result_from_raw(
    ...
    *,
    ...
    reference_policy = evaluate_reference_policy(sample, predictions)
    metrics = dict(reference_policy["metrics"])
    ...
    result = {
        ...
        "coordinate_system": "clip-local seconds after source clip extraction",
        "reference_evaluation": reference_policy,
...
def _write_report(
    ...
    *,
    ...
    hidden_gt = run_config.get("split") == "test"
    lines = [
        "# Experiment",
        "",
        "## Configuration",
        ...
        "",
        (
            f"This run contains {len(results)} Official-Test video; Ground Truth is hidden "
            "and was neither loaded nor evaluated."
            if hidden_gt
            else f"This dry run contains {len(results)} video from the QVHighlights community mirror."
        ),
        (
            "Official score: `UNAVAILABLE / HIDDEN_GT`."
            if hidden_gt
            else "The weak references are seed/teacher-derived development references, not official Ground Truth."
        ),
        "",
        "## Results",
        "",
    ...
    for item in results:
        ...
        if item.get("success"):
            ...
            if hidden_gt:
                details.insert(1, "- Official score: `UNAVAILABLE / HIDDEN_GT`")
            else:
                details[0:0] = [
    ...
    interpretation = (
        "Official-Test Ground Truth is hidden; no reference metric or official score was computed."
        if hidden_gt
        else "`temporal_iou` is the intersection duration divided by the union duration after separately merging overlapping prediction and weak-reference intervals. Precision, recall, and F1 are also duration-weighted. Degenerate weak references with duration at or below the configured epsilon remain in raw results but are excluded from duration-based denominators."
    )
    lines.extend(
        [
            "",
...
def main() -> int:
    ...
    if runtime_profile.uses_local_efficient_sdpa and args.local_model_path is None:
        raise SystemExit("local_efficient_sdpa requires --local-model-path")
    ...
    run_config = {
        ...
        "backend": "transformers" if args.local_model_path else "vllm",
        "transformers_version": _package_version("transformers"),
        "bitsandbytes_version": _package_version("bitsandbytes"),
        "quantization": args.local_quantization if args.local_model_path else None,
        "compute_dtype": args.local_compute_dtype if args.local_model_path else None,
        ...
        "dataset_version": args.dataset_version,
        "split": split,
        "reference_type": "hidden_ground_truth" if hidden_gt else "teacher_seed_weak_reference",
        "reference_identity": (
            "UNAVAILABLE / HIDDEN_GT"
            if hidden_gt
            else "Project-generated teacher/seed-derived weak-reference from QVHighlights public source windows; NOT official competition Ground Truth."
        ),
        "metric_policy": (
            "not evaluated; official score unavailable"
            if hidden_gt
            else "union-aware duration overlap; not an official competition metric"
        ),
    ...
    def get_pipeline() -> HighlightRetrievalPipeline:
        ...
        if client is None:
            if args.local_model_path:
                from aic_video_highlight.retrieval.qwen_transformers_client import (
                ...
                client = QwenTransformersClient(
                    ...
                    compute_dtype=args.local_compute_dtype,
        ...
        with tempfile.TemporaryDirectory(prefix=".aic-experiment-clip-", dir=source.parent) as temp:
            ...
            pipeline_result = get_pipeline().run(
                ...
                heartbeat_sink=(
                    ...
                    if runtime_profile.chunk_heartbeat
                    else None
                ),
                heartbeat_video_id=str(sample["video_id"]),
            )
        evaluation_started_at = time.perf_counter()
        reference_policy = evaluate_reference_policy(
            sample, _segment_dicts(pipeline_result.segments)
        )
        evaluation_sec = time.perf_counter() - evaluation_started_at
    ...
    results = run_with_failure_isolation(samples, process, component="sample_pipeline")
    ...
    for result in results:
        if result.get("success") is not True:
            continue
        if result.get("reference_evaluation", {}).get("status") != "AVAILABLE":
            continue
    ...
    if client is not None and hasattr(client, "stats"):
        stats = client.stats()
        backend_evidence = dict(stats.backend_evidence)
        atomic_write_json(
            output_dir / "local_backend_stats.json",
            {
                "backend": "transformers",
                "quantization": args.local_quantization,
                "compute_dtype": args.local_compute_dtype,
                "qwen_calls": stats.calls,
                "peak_vram_bytes": stats.peak_vram_bytes,
                "peak_vram_mib": stats.peak_vram_bytes / (1024**2),
                "peak_reserved_vram_bytes": stats.peak_reserved_vram_bytes,
                "peak_reserved_vram_mib": stats.peak_reserved_vram_bytes / (1024**2),
                "runtime_profile": stats.runtime_profile,
                "attention_backend": stats.attention_backend,
                "gqa_execution_mode": stats.gqa_execution_mode,
                "torch_version": stats.torch_version,
                "cuda_version": stats.cuda_version,
                "gpu_name": stats.gpu_name,
                "backend_evidence": backend_evidence,
                "call_records": list(stats.call_records),
                "model_inference_sec": sum(
                    float(item.get("timing", {}).get("model_inference_sec", 0.0))
                    for item in results
                ),
                "total_sec": sum(
                    float(item.get("timing", {}).get("total_sec", 0.0))
                    for item in results
                ),
            },
        )
